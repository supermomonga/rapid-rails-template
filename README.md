...
アプリ内通知は選択肢を持たない常設機能です。全構成へ通知・配信model、管理CRUD、ヘッダーの遅延読込popover、25件単位の通知履歴、個別・一括既読操作を生成します。Web Pushを有効にした場合の購読設定はアプリ内通知と分離し、`/web-push`の「Web Push設定」画面に生成します。
