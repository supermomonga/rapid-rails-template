...
デプロイは全構成でKamal V2に固定し、`production`・`staging` destinationを必須にします。Litestream 0.5.15は単一Linux host上のAccessoryとして、destination別SQLite volumeをCloudflare R2の別bucketへ複製します。生成される`deployment:configure`は、人間ユーザーとしてログインした1Password accountを選び、対象destinationのvault・資格情報item、読み取り専用service account、R2 bucket、bucket限定Cloudflare tokenを一括で事前調査します。既定値が「中止」の確認後だけ不足資源を作成し、完全一致する既存資源をIDで再利用します。環境別service accountのtokenは同じ環境vault内の専用itemへ保存し、ローカル設定やKamalの取得対象には含めません。空volumeの起動時復元は既存DBを上書きせず、既存DBを戻す操作はdestination必須の`bin/kamal-restore`でdry-run、TTY、完全一致確認、全DB整合性検査、deploy lock、復元前ファイル保存を必須にします。詳細は生成アプリの`docs/deployment.md`へ出力します。
