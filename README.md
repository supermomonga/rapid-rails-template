...
デプロイは全構成でKamal V2に固定し、`production`・`staging` destinationを必須にします。Litestream 0.5.15は単一Linux host上のAccessoryとして、destination別SQLite volumeをCloudflare R2の別bucketへ複製します。生成される`litestream:configure:r2`は1Password service account専用で、初回に人間ユーザーからaccount全体でproduction/staging vaultを再利用または作成し、その両方へ明示的な読み書き権限を持つservice accountのtokenをgit管理外の`mise.local.toml`へ保存して終了します。再実行時はvaultを追加作成せず、対象accountのSuper Administratorが作成したdashboard上の`Account API Tokens: Edit`（API上の`Account API Tokens Write`）だけを持つ`CLOUDFLARE_INITIAL_API_TOKEN`を、1Password・R2・repositoryへの変更前にactive状態・所有account・policyまで検証します。検証後にdestination別・bucket限定のCloudflare tokenを冪等に作成または再利用し、token本体と派生資格情報をdestination別1Password vaultだけへ保存してKamal secret参照を構成します。空volumeの起動時復元は既存DBを上書きせず、既存DBを戻す操作はdestination必須の`bin/kamal-restore`でdry-run、TTY、完全一致確認、全DB整合性検査、deploy lock、復元前ファイル保存を必須にします。詳細は生成アプリの`docs/deployment.md`へ出力します。
