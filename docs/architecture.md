...
- Kamalは`production`・`staging` destinationを必須とし、LitestreamのCloudflare R2 bucket、資格情報、volume、restore markerをdestination単位で分離する
...
- SorbetとTapiocaが全構成へ常設され、`db/seeds.rb`を含むapplication codeが`typed: true`以上、純粋なserviceが`typed: strict`となり、通常のRails testが型エラー、未解決定数、Gem／Rails DSL RBIの更新忘れ、重複shimを非破壊検出すること。
