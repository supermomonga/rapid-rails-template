...
現在は設計段階です。この文書に示す`bootstrap.rb`、`src/`、`bin/`、`test/`は実装開始時に追加し、空ディレクトリやプレースホルダーは先行して作成しません。
...
固定構成と正規化済み回答から、順序付きの`rails new`引数配列を構築します。shell展開される文字列は生成せず、確認画面へ同じ内容を表示します。
