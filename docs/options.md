...
生成する`litestream:configure:r2`は1Password service account専用です。`OP_CONNECT_HOST`または`OP_CONNECT_TOKEN`がある場合は認証優先順位の衝突として失敗します。`OP_SERVICE_ACCOUNT_TOKEN`がなく、人間ユーザーとして1Password CLIへログイン済みの場合は、確認後にvault作成権限付きの`dev:<正規化済みapp_id>`を作成し、tokenを`mise set --stdin`でgit/Docker管理外の`mise.local.toml`へ保存して`0600`へ変更し、その実行を終了します。tokenは平文のローカル設定であり、1Passwordが推奨する非平文保存とは異なります。上書きしても旧service accountは失効・削除しません。以後の設定・setup・deploy・accessory・restore操作は、tokenを確実に読むため`mise exec --`経由で実行します。

再実行時は`op user get --me`とKamal adapter相当の`op account get --account=<service-account-id>`で有効なservice account認証を検証します。productionには`<正規化済みapp_id>-production`、stagingには`<正規化済みapp_id>-staging`というservice accountから見える専用vaultを使い、未作成ならdestinationごとに確認後に作成します。ローカルWrangler v4とGumを使い、ログイン中のCloudflare identity、account、production/staging、bucket、service account ID、destination別vault ID、item、Kamal secret参照の適用予定を表示します。bucket名の既定値は`<正規化済みapp_id>-db-production`と`<正規化済みapp_id>-db-staging`とし、確認前に変更できます。既定で両destinationを選択し、既定値「中止」の全体確認後だけ未存在bucket、1Password item、`.kamal/secrets.<destination>`を変更します。先に作成したservice accountやvaultは中止時も保持します。資格情報本体はJSON標準入力でdestination別vaultの1Password itemへ保存し、repositoryやargvへ書きません。
