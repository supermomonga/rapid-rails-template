...
`CLOUDFLARE_INITIAL_API_TOKEN`には`Account API Tokens: Edit`だけを持つaccount-owned Initial Tokenを毎回環境変数から渡します。未設定なら、最小権限を指定済みのToken Template URLとCloudflare公式ドキュメントを表示し、Cloudflare、1Password、bucket、repositoryを変更せず正常終了します。Initial Tokenは1Passwordや`mise.local.toml`へ保存しません。
