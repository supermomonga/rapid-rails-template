...
Wranglerのaccount選択直後にInitial Tokenのverify、token詳細、permission groupを照合し、activeであること、選択accountに属すること、そのaccountだけをresourceとして`Account API Tokens Write`だけを持つことを検証します。HTTP 401・403、account不一致、失効、Readだけ、余分な権限は、対象accountと再発行用Token Template URLを示して、1Password vault・service account・R2 bucket・repository fileを変更する前に失敗させます。

ローカルWrangler v4とGumを使い、ログイン中のCloudflare identityとaccount、選択した1Password accountと人間ユーザー、対象destination、vault、service accountとtoken item、bucket、R2資格情報item、Kamal secret参照、`<正規化済みapp_id>-r2-<destination>`というCloudflare token名、各資源の作成・再利用をすべて事前に表示します。120文字を超えるtoken名はapp-id部分を短縮し、元app-idのSHA-256 hashを付けます。bucket名の既定値は`<正規化済みapp_id>-db-production`と`<正規化済みapp_id>-db-staging`とし、確認前に変更できます。既定で両destinationを選択し、既定値「中止」の確認を1回だけ行い、承認後にvault、service account、bucket、Cloudflare API token、1Password item、`.kamal/secrets.<destination>`の順で適用します。中止時は外部資源とrepository fileを変更しません。

Cloudflare APIからpermission group IDを解決して対象bucketだけのallow policyを作成し、同名tokenを全ページ検索します。Cloudflare tokenと1PasswordのR2資格情報itemが両方ない場合だけ、最終確認済みかつbucket作成済みで新規作成します。同名tokenが1件あり、active、policy、bucket、1Password itemのcategory、5つの管理fieldの型と値、token ID・token本体・SHA-256派生secretが一致する場合は、両方を「既存を使用」と表示し、既存item IDを再利用して`op item edit`を実行しません。tokenとitemの片方だけが存在する場合、同名重複、無効token、policy差異、vault内secret欠落や不一致では、追加作成・上書き・rotation・revoke・削除をせず失敗します。作成したtokenの`id`を`R2_ACCESS_KEY`、`value`のSHA-256を`R2_SECRET_KEY`として、token本体を`CLOUDFLARE_R2_API_TOKEN`としてdestination別1Password itemへJSON標準入力で保存します。service account tokenまたはCloudflare tokenのitem保存が失敗した場合は、同名itemを再取得して曖昧な成功を確認してから再試行します。打ち切る場合だけ二重確認し、手動保存用の値を`/dev/tty`へ一度表示します。secretはargv、通常出力、repositoryへ書きません。`.kamal/secrets.production`と`.kamal/secrets.staging`は選択した1Password account ID、vault ID、R2 item IDを参照して従来の4項目だけを取得し、service account tokenと`CLOUDFLARE_R2_API_TOKEN`をデプロイ先へ渡しません。
