...
依存関係のインストール後、公式generatorと設定APIを実行します。`shadcn_view_components`のinstall generatorでTailwind CSSとStimulusへコンポーネントを登録し、Rails標準を基準とするscaffold controller／Viewとcontroller View templateを配置します。scaffold indexはcontrollerでPagyを適用し、共通`pagination` helperが`Shadcn::Pagination`を描画します。Passkeyのsignup・login Viewと、アカウント設定内のPasskey CRUD画面を常設し、SIWE選択時だけsignup・login導線とEVMウォレットCRUD画面を追加します。登録画面に名称入力は置かず、signupと追加登録の検証済みcredentialからserverが初期名を決めます。SIWEは全ceremonyでEIP-6963 Providerを収集し、1件なら直接接続、複数なら名前一覧dialogで選択し、非対応時だけ`window.ethereum`へ接続します。検証済みcredentialをsessionの認証元として記録し、SIWEで現在使用中のwalletと最後のログイン方法には一覧の解除導線を出さず、serverでも解除を拒否します。資格情報の名称変更はedit、別資格情報による対象外再認証を伴う解除はshowへ分離します。別URLへ移動するページ内ナビゲーションは共通`with_tab` helperから`Shadcn::NavigationMenu`で描画し、native dialogは共通`with_modal` helperから`Shadcn::Dialog`で生成します。各Viewで同じDOM契約を再構築しません。各画面は主見出しを`content_for :page_title`へ1回だけ設定し、accountとadminは共通`with_menu` layoutを使用します。Tailwind CSS utilityはresponsive layoutとDESIGN固有の調整に限定し、構造化APIで表現できないRubyコード編集にはPrismを使用します。
...
通常のアプリ生成ではブラウザを起動しませんが、test用の`evidence:capture` Rake taskと撮影runnerを生成します。`rake evidence:update`はアプリ内通知、SIWE、PWA、Web Push、Solid Queue、管理者向け運用画面、全Profile機能、API、Solid Cable、mailを有効にしたKamal対応の日本語sampleを1回だけ生成します。同じアプリで全Rails test、Sorbet・RBI検証、RuboCop、Thruster cacheの`miss`から`hit`への遷移を検証し、運営者情報の公開表、通知popoverの読込中、「あなたへの通知」と「お知らせ」の未読状態・既読後、両タブの通知履歴、通知管理、通知編集、個別受信者選択、Passkey、SIWE、avatarのシナリオを撮影して整合性検証が成功した場合だけ`docs/evidence/`を置換します。全体通知の配信行が0件であることも撮影前に検査します。運営者情報の表はmobileでセル内へ折り返してpage全体に横overflowを発生させないことをcomputed geometryで検査します。通知headerとpopoverは320、390、640、960、961pxでviewport内へ収まり、page全体に横overflowがないこともcomputed geometryで検査します。
