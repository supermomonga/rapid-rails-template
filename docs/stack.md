...
model、policy、service、job、mailer、validator、application-owned `lib`は生成時に`# typed: true`を先頭へ付与します。全機能構成ではこのdomain層が31ファイルとなり、公開class method、instance method、constructor、job、validator、policy predicateへinline signatureを置きます。private callbackは推論に任せます。Action Policyはgem側の`method_added` hookがSorbet runtime hookへ委譲しないため、policy predicateだけSorbet公式の`T::Sig::WithoutRuntime.sig`を使用し、静的なinline signatureを保ちます。`typed: ignore`による除外は行いません。
