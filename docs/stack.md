...
- `avatar`選択時はCropper.js 2.1.1をImportmapの公式`pin` commandでtransitive dependencyごと`vendor/javascript`へ固定し、実行時CDNとJavaScript bundlerを使用しない。`avatar_crop` Stimulus controllerは1:1の可変selection、画像移動、zoom、reset、512×512 canvas出力、File置換、Object URLとCropper lifecycleを担当する。変換前のraw画像はformへ残さず、失敗時にraw uploadへfallbackしない。
