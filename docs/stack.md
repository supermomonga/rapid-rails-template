...
SSHは22番、root、公開鍵認証、`StrictHostKeyChecking=accept-new`に固定します。`/etc/ssh/sshd_config.d/99-rapid-rails-template.conf`でpasswordとkeyboard-interactiveを無効化し、公開鍵root loginを維持します。変更中はControlMaster接続を保持し、`sshd -t`、`sshd -T`、独立した新規接続が失敗した場合は旧drop-inを復元します。FirewallとOS packageは変更しません。

Cloudflare token作成用の`CLOUDFLARE_INITIAL_API_TOKEN`は、対象accountのSuper Administratorが作成し、dashboard上の`Account API Tokens: Edit`（API上の`Account API Tokens Write`）だけを持たせて実行環境から毎回渡し、保存しません。作成確認画面では対象accountと権限を確認します。未設定なら最小権限のToken Template URLを表示して無変更で正常終了します。Wranglerのaccount選択直後にtokenのactive状態、所有account、単一account resource、単一permissionをAPIで照合し、不一致や認証拒否では1Password・R2・repositoryを変更する前に停止します。destinationごとのtoken名は`<正規化済みapp_id>-r2-<destination>`、権限は対象bucketの`Workers R2 Storage Bucket Item Write`だけです。同名tokenと1Password itemがcategory、管理fieldの型と値を含めて完全一致する場合だけ両方を再利用し、1Password itemを再保存しません。片方だけが存在する場合や、重複・無効・policy差異・secret欠落・不一致では、自動上書き・rotation・revoke・削除をせず停止します。Cloudflare token本体は1Password itemの`CLOUDFLARE_R2_API_TOKEN`へconcealed fieldとして保存し、token IDとtoken本体のSHA-256から派生したS3資格情報も同じitemへ保存します。Kamalが取得するのは`CF_ACCOUNT_ID`、`LITESTREAM_R2_BUCKET`、`R2_ACCESS_KEY`、`R2_SECRET_KEY`だけです。
