...
databaseとannotationを確定し、config DSLのreceiverをRuby本体の`T.bind`で接続してからTapiocaを初期化します。この順序により、Rails CIの実体でない未解決の`CI`定数を`todo.rbi`へ生成しません。test databaseを準備してtest環境のRails DSL RBIを全体生成した後、controller、concern、helper、model、policy、service、job、mailer、validator、application-owned `lib`、config、testの先頭へ`# typed: true`を付与します。`sorbet/rbi/shims/framework_bindings.rbi`を生成してからSorbet検証へ進み、migration、schemaだけは`typed: false`に留めます。DSL／Gem／annotation RBIは生成物として手動編集せず、アプリ定義methodはRuby本体のinline signature、限定的なframework wiringだけはshimで表現します。
