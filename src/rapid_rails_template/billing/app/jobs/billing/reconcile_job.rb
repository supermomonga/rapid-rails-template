...
module Billing
  class ReconcileJob < ::ApplicationJob
    ...
    private
      ...
      def process_subscription(subscription)
      ...
      rescue GasLimitExceeded, RpcError, ConfigurationError => error
        if charge
          charge.update!(status: "held", hold_reason: T.must(error.class.name).demodulize, next_attempt_at: 1.hour.from_now)
        ...
        Event.admin!("subscription:#{subscription.id}:#{error.class.name}", "collection_held")
      rescue VerificationError
