  ...
  class ChargeBuilder
    def self.call(subscription, now: Time.current)
      ...
      settings.with_lock do
        account = subscription.plan.merchant_account
        account.lock!
        subscription.lock!
        return if subscription.ended_at || subscription.cancel_requested_at || subscription.signature.blank?
        return unless settings.payments_enabled? && account.status == 'active'
        ...
        basis_points = account.effective_fee_basis_points(setting: settings)
        fee, merchant = Amount.split(subscription.amount_units, basis_points)
        destination = account.payout_addresses.find_by!(chain_id: subscription.chain_id).address
