...
class EvidenceCapture
  private
    def capture_billing_scenarios(viewport)
      merchant = Billing::MerchantProfile.find_or_create_by!(user: @user) do |record|
        record.public_id = "sample-studio"
      ...
      authenticate
      capture_page("billing-merchant-overview-empty", "販売者画面・初回の販売準備", routes.merchant_root_path, translate("billing.ui.merchant_area"), viewport)
      capture_page("billing-merchant-edit", "販売者プロフィール", routes.edit_merchant_profile_path, translate("billing.ui.merchant_settings"), viewport)
      capture_page("billing-payouts", "チェーン別の販売者送金先", routes.merchant_payout_addresses_path, translate("billing.ui.merchant_settings"), viewport)
      capture_page("billing-seller-plans", "販売プラン一覧", routes.merchant_plans_path, translate("billing.ui.seller_plans"), viewport)
      capture_page("billing-plan-new", "販売プラン作成", routes.new_merchant_plan_path, translate("billing.ui.new_plan"), viewport)
      capture_page("billing-plan-edit", "販売プラン編集", routes.edit_merchant_plan_path(plan), translate("billing.ui.edit_plan"), viewport)
      ...
      capture_page("billing-operator-plans", "運営者プラン一覧", routes.admin_plans_path, translate("billing.ui.operator_plans"), viewport)
      capture_page("billing-operator-plan-new", "運営者プラン作成", routes.new_admin_plan_path, translate("billing.ui.new_plan"), viewport)
      ...
      capture_page("billing-merchant-overview", "販売者画面・売上概要", routes.merchant_root_path, translate("billing.ui.merchant_area"), viewport)
      ...
      capture_page("billing-sales", "販売契約一覧", routes.merchant_sales_path, translate("billing.ui.sales"), viewport)
      capture_page("billing-payments", "販売者の決済履歴", routes.merchant_payments_path, translate("billing.ui.sales"), viewport)
      capture_page("billing-refunds", "販売者の手動返金履歴", routes.merchant_refund_records_path, translate("billing.ui.sales"), viewport)
      capture_page("billing-sale", "販売契約詳細", routes.merchant_sale_path(contract), translate("billing.ui.contract", id: contract.id), viewport)
      capture_page("billing-refund-new", "手動返金の記録フォーム", routes.new_merchant_charge_refund_record_path(charge), translate("billing.ui.manual_refund"), viewport)
      ...
      verify_billing_navigation(routes, viewport)
      ...
      capture_page("billing-merchant-new", "販売者登録", routes.new_merchant_profile_path, translate("billing.ui.merchant_registration"), viewport)
      Capybara.reset_sessions!
      page.current_window.resize_to(viewport_size.fetch("width"), viewport_size.fetch("height"))
      authenticate
      if viewport == "mobile"
    ...
    def verify_billing_navigation(routes, viewport)
      visit routes.merchant_sales_path
    ...
    def billing_evidence_contract(plan, viewport)
      now = (Time.current - 30.days + 1.hour).change(usec: 0)
      ...
      plan.subscriptions.create!(user: @user, seller_kind: "merchant", buyer_name: "Evidence User", seller_name: plan.seller_name,
        plan_name: plan.name, chain_id: 8453, payer_address: permission.fetch("account"), amount_units: 10_000_000,
        period_seconds: 30 * 86400, starts_at: now, permission: permission, permission_hash: "0x#{viewport == 'desktop' ? 'dd' * 32 : 'ee' * 32}")
    end
end
